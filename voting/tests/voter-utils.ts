...
export function createAbstainedEvent(
    ...
    abstainedEvent.parameters.push(new ethereum.EventParam("totalWeight", ethereum.Value.fromUnsignedBigInt(totalWeight)));
...
export function createGaugeCreatedEvent(
    ...
    gaugeCreatedEvent.parameters.push(new ethereum.EventParam("votingRewardsFactory", ethereum.Value.fromAddress(votingRewardsFactory)));
    gaugeCreatedEvent.parameters.push(new ethereum.EventParam("gaugeFactory", ethereum.Value.fromAddress(gaugeFactory)));
    gaugeCreatedEvent.parameters.push(new ethereum.EventParam("pool", ethereum.Value.fromAddress(pool)));
    gaugeCreatedEvent.parameters.push(new ethereum.EventParam("bribeVotingReward", ethereum.Value.fromAddress(bribeVotingReward)));
    gaugeCreatedEvent.parameters.push(new ethereum.EventParam("feeVotingReward", ethereum.Value.fromAddress(feeVotingReward)));
...
export function createVotedEvent(voter: Address, pool: Address, tokenId: BigInt, weight: BigInt, totalWeight: BigInt, timestamp: BigInt): Voted {
...
export function createWhitelistTokenEvent(whitelister: Address, token: Address, _bool: boolean): WhitelistToken {
    ...
    whitelistTokenEvent.parameters.push(new ethereum.EventParam("whitelister", ethereum.Value.fromAddress(whitelister)));
